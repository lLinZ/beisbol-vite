...
import { DescripcionDeVista } from '../../components/layout/content/DescripcionDeVista'
...
export const ProfilePage = (props: Props) => {
    const { authState, editData } = useContext(AuthContext);
    ...
    const onSubmit = (values: FormikValues, resetForm: (nextState?: Partial<FormikState<ChangeData>> | undefined) => void) => {
        ...
        const phone = values.phone ? values.phone : false;
        const password = values.password ? values.password : false;
        const confirmarPassword = values.confirmarPassword ? values.confirmarPassword : false;
        editData(email, phone, password, confirmarPassword);
    }
                ...
                <Grid item xs={12} sx={styles.gridItem} >
                    <Box sx={styles.colorPickerContainer}>
                        <Box sx={{ display: "flex", flexFlow: "row wrap", alignItems: "center" }}>
                            {authState.id !== 0 ? (<Avatar sx={{ bgcolor: authState.color ? authState.color : blue[700], mr: 2 }}>{authState.name.substring(0, 1)}</Avatar>) : (<></>)}
                        ...
                        {({ values, handleChange, handleSubmit }) => (
                            <Form onSubmit={handleSubmit}>
                                <Grid container spacing={2}>
                                    ...
                                    <Grid item xs={12} sm={6}>
                                        <TextFieldCustom label="Telefono" name='phone' onChange={handleChange} />
                                    </Grid>
                                    <Grid item xs={12} sm={6}>
